...
if (!function_exists('lc_backlink_cps_campaign_definition')) {
    /**
     * 백링크 CPS — ONOFF Core BACKLINK + LIFETIME 규칙.
     *
     * @return array<string,mixed>
     */
    function lc_backlink_cps_campaign_definition()
    {
        return array(
            'code'               => 'CPS-BACKLINK',
            'title'              => '백링크',
            ...
            'description'        => 'ONOFF 백링크 서비스 홍보. 결제·이용 확정 시 Core LIFETIME 수수료(10%)가 적용됩니다. https://backlink.icrm.co.kr/',
            'badge'              => '신규',
            'recommended'        => true,
            'status'             => 'active',
            'platform_service'   => 'BACKLINK',
            'product_type'       => 'lifetime',
            'commission_rule_id' => 'global_lifetime_default_10pct',
            'landing_url'        => 'https://backlink.icrm.co.kr/',
        );
    }
}
...
if (!function_exists('lc_campaign_ensure_backlink_cps')) {
     ...
     */
    function lc_campaign_ensure_backlink_cps(array $options = array())
    {
        if (!lc_db_installed()) {
        ...
        $def = lc_backlink_cps_campaign_definition();
        ...
        if ($cp_id <= 0) {
            return array('ok' => false, 'message' => '백링크 CPS 캠페인 생성에 실패했습니다.');
        }
