...
if (!function_exists('lc_domain_cps_campaign_definition')) {
    /**
     * 낙장도메인(CatchDomain) CPS — ONOFF Core DOMAIN + LIFETIME 규칙.
     *
     * @return array<string,mixed>
     */
    function lc_domain_cps_campaign_definition()
    {
        return array(
            'code'               => 'CPS-DOMAIN',
            'title'              => '낙장도메인',
            ...
            'description'        => '낙장·만료 도메인 검색·해제 서비스(CatchDomain). 결제·이용 확정 시 Core LIFETIME 수수료(10%)가 적용됩니다. https://domain.icrm.co.kr/',
            'badge'              => '신규',
            'recommended'        => true,
            'status'             => 'active',
            'platform_service'   => 'DOMAIN',
            'product_type'       => 'lifetime',
            'commission_rule_id' => 'global_lifetime_default_10pct',
            'landing_url'        => 'https://domain.icrm.co.kr/',
        );
    }
}
...
if (!function_exists('lc_campaign_ensure_domain_cps')) {
    ...
    function lc_campaign_ensure_domain_cps(array $options = array())
    {
        ...
        $def = lc_domain_cps_campaign_definition();
        ...
        if ($cp_id <= 0) {
            return array('ok' => false, 'message' => '낙장도메인 CPS 캠페인 생성에 실패했습니다.');
        }

        return array(
            'ok'      => true,
            'message' => '낙장도메인 CPS 캠페인을 등록했습니다.',
            'cpId'    => $cp_id,
            'created' => true,
            'mtId'    => $mt_id,
        );
